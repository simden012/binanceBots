...
class binance_user:
    ...
    def order_market_buy(self, symbol, quantity):
        ...
        if (order['status'] == 'FILLED'):
            print(
                f"Buy Order for {symbol} was filled for this quantity: {quantity} so {self.quantity_to_price(symbol, quantity)}$")
        return order
    ...
    def order_market_sell(self, symbol, quantity):
        ...
        if (order['status'] == 'FILLED'):
            print(
                f"Sell Order for {symbol} was filled for this quantity: {quantity} so {self.quantity_to_price(symbol, quantity)}$")
        return order
    ...
    def limit_order(self, ticker, side, quantity, price):
        ...
        if (order_limit['status'] == 'NEW'):
            print(
                f"Limit Order for {ticker} was created for this quantity: {quantity} and this price: {price}")
        return order_limit
